...
export async function generateImage(request: GenerateRequest, signal?: AbortSignal): Promise<RunLog> {
  ...
  try {
    ...
    if (request.provider === "openai") {
      ...
      const response = await fetchWithTimeout(`${GMS_URLS.openaiBase}/images/generations`, {
        method: "POST", signal,
      ...
      usage = calculateUsage("image", request.model, usageCount(apiUsage?.input_tokens), usageCount(apiUsage?.output_tokens));
      responseSummary = { created: body.created, imageCount: items.length, usage: apiUsage || null };
    } else {
      ...
      usage = calculateUsage("image", request.model, usageCount(meta.promptTokenCount), usageCount(meta.candidatesTokenCount));
      responseSummary = { finishReasons: candidates.map((candidate: any) => candidate.finishReason).filter(Boolean), imageCount: payloads.length, usageMetadata: meta };
    }
