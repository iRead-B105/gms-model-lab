...
test("브라우저 새로고침과 HMR 중에도 유료 생성 작업을 끝까지 저장한다", async () => {
  const [imageRoute, textRoute, ttsRoute, textGeneration, common, page, results] = await Promise.all([
    source("app/api/generate/route.ts"),
    source("app/api/text/generate/route.ts"),
    source("app/api/tts/generate/route.ts"),
    source("lib/server/gms/text-generation.ts"),
    source("lib/server/gms/common.ts"),
    source("app/page.tsx"),
    source("features/model-lab/components/results.tsx"),
  ...
  assert.match(imageRoute, /generateImage\(input\)/);
  assert.match(textRoute, /createTextStream\(input\)/);
  assert.match(ttsRoute, /generateSpeech\(input\)/);
  assert.doesNotMatch(`${imageRoute}${textRoute}${ttsRoute}`, /\(input, request\.signal\)/);
  assert.doesNotMatch(textGeneration, /cancel\(\) \{ clientClosed = true; localAbort\.abort/);
  assert.match(common, /원인을 확인할 수 없는 빈 오류/);
  assert.match(page, /setInterval\(\(\) => void loadLogs\(\), 5_000\)/);
  assert.match(results, /GMS 생성 성공 · 로컬 이미지 없음/);
});

test("로컬 기록은 서버 페이지네이션과 전체 검색 결과 내보내기를 지원한다", async () => {
  const [route, query, page, pagination, readme] = await Promise.all([
    source("app/api/logs/route.ts"),
    source("lib/server/storage/run-query.ts"),
    source("app/page.tsx"),
    source("features/model-lab/components/log-pagination.tsx"),
    source("README.md"),
  ]);

  assert.match(route, /searchParams/);
  assert.match(route, /positiveInteger/);
  assert.match(route, /queryRuns/);
  assert.match(query, /LOG_PAGE_SIZES = \[10, 25, 50, 100\]/);
  assert.match(query, /Math\.max\(1, Math\.ceil\(filteredRuns\.length \/ pageSize\)\)/);
  assert.match(query, /Math\.min\(totalPages, Math\.max\(1, query\.page\)\)/);
  assert.match(query, /filteredRuns\.slice\(\(page - 1\) \* pageSize, page \* pageSize\)/);
  assert.match(query, /query\.status/);
  assert.match(query, /normalizedSearch/);
  assert.match(page, /<LogPagination/);
  assert.match(page, /for \(let page = 2; page <= firstPage\.totalPages; page \+= 1\)/);
  assert.match(pagination, /첫 페이지/);
  assert.match(pagination, /마지막 페이지/);
  assert.match(readme, /기록 서버 페이지네이션/);
});
